...
export const MyBooking = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(
        `https://hotel-booking-platform-server.vercel.app/myBookings/${user.email}`
      );
  ...
  const {
    ...
    queryFn: async () => {
      const res = await axios.get(
        `https://hotel-booking-platform-server.vercel.app/bookings/${id}`
      );
      return res.data;
    },
  });
  ...
  const handleUpdatebooking = (e) => {
    ...
    axios
      .patch(
        `https://hotel-booking-platform-server.vercel.app/updateBooking`,
        bookingData
      )
      .then(() => {
        refetch();
  ...
  const handleCancelBooking = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            `https://hotel-booking-platform-server.vercel.app/cancelbooking/${id}`
          )
          .then(() => {
            refetch();
  ...
  const handleReviewSubmit = () => {
    ...
    axios
      .post(
        `https://hotel-booking-platform-server.vercel.app/givereview`,
        reviewinfo
      )
      .then((res) => {
              ...
              <tbody>
                ...
                {bookings.map((booking) => (
                  <tr key={booking._id}>
                    ...
                    <td>{booking.cost_per_night}</td>
                    <td>
                      <button
                        onClick={() =>
                          document.getElementById("my_modal_3").showModal()
                        }
                        className="btn btn-sm text-sm bg-blue-100"
                      >
                        Review
                      </button>
                    </td>
